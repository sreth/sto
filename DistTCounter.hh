...
class DistTCounter : public TObject {
...
public:
    ...
    DistTCounter<T, W>& operator=(T x) {
        Sto::item(this, 0).add_write(x).assign_flags(assigned_bit);
        return *this;
    }
    DistTCounter<T, W>& operator=(const DistTCounter<T, W>& x) {
        return *this = x.operator T();
    }
    template <typename TT, typename WW>
    DistTCounter<T, W>& operator=(const DistTCounter<TT, WW>& x) {
    ...
    DistTCounter<T, W>& operator+=(T delta) {
        auto item = Sto::item(this, 0);
...
bool operator==(T a, const DistTCounter<T, W>& b) {
    return b == a;
}
template <typename T, typename W>
bool operator!=(T a, const DistTCounter<T, W>& b) {
